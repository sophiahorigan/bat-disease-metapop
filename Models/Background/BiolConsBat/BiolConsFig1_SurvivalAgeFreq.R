...
dat.new <- read.csv(file = paste0(madabatwd, "/Catching-Data/Historic-Catching-Data-Through2020/catching_data_historic.csv"), header = T, stringsAsFactors = F)
head(dat.new)
...
plot.hist <- function(tot.dat, do.save, filename){
  ...
  p1 <- ggplot() + geom_bar(data=hist.count, aes(x=hist_year, y = count), stat="identity", fill="white", col="black") + 
    ...
    xlab("age(yrs)") + ylab("frequency") + theme_bw() + coord_cartesian(xlim=c(0, 15), ylim = c(0,100)) + 
...
constant.exp.fit(species1 = "Eidolon dupreanum", tot.dat = dat.age, adult_fecundity = (  0.23), do.plot = T) 
constant.exp.fit(species1 = "Eidolon dupreanum", tot.dat = dat.age, adult_fecundity = (  0.2111), do.plot = T) #if adult fecundity really is half of P. rufus, these guys could be in serious trouble too...but this is likely not the case
...
constant.exp.fit(species1 = "Pteropus rufus", tot.dat = dat.age, adult_fecundity = 0.3730, do.plot = T)
constant.exp.fit(species1 = "Pteropus rufus", tot.dat = dat.age, adult_fecundity = 0.4330, do.plot = T)
...
plot.hist.model <- function(tot.dat, mod.dat, do.save, filename){
  ...
  p1 <- ggplot() + geom_bar(data=hist.count, aes(x=hist_year, y = count), stat="identity", fill="white", col="black") + 
    ...
    geom_label(data=mod.dat, aes(x=14, y=80, label =sj), parse=T, label.size = NA, col="red")+
    geom_label(data=mod.dat, aes(x=14, y=90, label =sa), parse=T, label.size = NA, col="red")+
    geom_label(data=mod.dat, aes(x=14, y=100, label =Fa), parse=T, label.size = NA, col="red")+
    geom_ribbon(data=mod.stab.age, aes(x=hist_year, ymax=count_uci, ymin=count_lci), fill="red", alpha=.3)+
    xlab("age(yrs)") + ylab("frequency") + theme_bw() + coord_cartesian(xlim=c(0, 15), ylim = c(0,105)) + 
    theme(axis.title = element_text(size=30, face = "plain"), panel.grid = element_blank(), 
          axis.text = element_text(size=25), title = element_text(size = 30, face="italic"),
          strip.text = element_text(face = "italic", size = 20), strip.background = element_blank())   
  print(p1)
...
par(cex = (1.2))
plot(density(dat$age), ylab="Density", xlab=c("Age at death"), xlim=c(0,15), ylim=c(0,.5), col="blue", main = NA,lwd=2)
lines(density(age.at.death.mod.Eid), col="red", lwd=2)
legend(x=8.5,y=.5, legend = c("modern\n(modeled)", "subfossil"), col=c("red", "blue"), bty="n", lwd=1.5, cex = .8)
dev.off()
