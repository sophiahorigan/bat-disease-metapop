...
#and make a second vector of the newer data
bat.demo.new = bat.demo
#overwrite survival rates with new estimates
bat.demo.new$IBI_fecundity <- c(.48, .2205, .48, .424)
bat.demo.new$infant_annual_survival <- c(.5866705, 1.277,4.519, 5.115)
bat.demo.new$adult_annual_survival <- c(.7802735,.7802735,.3155653,.3155653)
...
wrap.isocline.bats.two.lines <- function(dat1, dat2, type, p_sequence, do.plot, do.save, filename){
  
 
  
  dat1$adult_annual_mortality <- 1- dat1$adult_annual_survival 
  dat1$infant_annual_mortality <- 1- dat1$infant_annual_survival 
  dat1$infant_annual_mortality[dat1$infant_annual_mortality<0] <- 0
  #dat1$infant_annual_mortality[dat1$infant_annual_mortality<0 & dat1$scenario=="worst"] <- dat1$infant_annual_mortality[dat1$species=="Eidolon dupreanum" & dat1$scenario=="worst"]
  
  dat2$adult_annual_mortality <- 1- dat2$adult_annual_survival 
  dat2$infant_annual_mortality <- 1- dat2$infant_annual_survival 
  dat2$infant_annual_mortality[dat2$infant_annual_mortality<0] <- 0
  ...
  if(do.plot==TRUE){
    ...
    dat2 =subset(dat2, scenario=="best")
    ...
    p <-  ggplot(data=isocline.df) + 
      ...
      geom_segment(data=dat1, aes(x=adult_annual_mortality, y=0, xend=adult_annual_mortality, yend=1), linetype=2, size=1, color="magenta") +
      geom_point(data=dat1, aes(x=adult_annual_mortality, y=infant_annual_mortality), shape=18, size=6, color="magenta") +
      geom_segment(data=dat2, aes(x=adult_annual_mortality, y=0, xend=adult_annual_mortality, yend=1), linetype=2, size=1, color="limegreen") +
      geom_point(data=dat2, aes(x=adult_annual_mortality, y=infant_annual_mortality), shape=18, size=6, color="limegreen")
...
wrap.isocline.bats.two.lines(dat1=bat.demo, dat2=bat.demo.new, type="isocline", p_sequence=seq(0, 1, .01), do.plot=TRUE, 
                   do.save=TRUE, filename=paste0(homewd, "/prior-scripts/BiolConsBat/figures/bat_isoclines_compare_old_new_mort.pdf"))
...
mat.Eid = matrix(c(0,(.48*.780),.587,.780 ), byrow=T, nrow=2)
mat.Pter = matrix(c(0,(.48*.316),.587,.316 ), byrow=T, nrow=2)
Re(eigen(mat.Eid)$values)[1] #.9998
Re(eigen(mat.Pter)$values)[1] #.4956
