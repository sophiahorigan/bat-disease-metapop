...
sim.met.MSIR.age <- function(burnin, sim_pop, yrs, ntyr, age.brk, s, beta, recov, mort, mort_juv, adult_fec, wane, slope.wane, sigma, mu.sick, add.inf.mort){
  ...
  for (i in 1:(length(times)-1)){
    ...
    #need to also feed it biweek because vaccination occurs only during certain biweeks!
...
run.sim.MSIRS <- sim.met.MSIR.age(burnin=30, 
                                  sim_pop=10000, 
                                  yrs=50, 
                                  ntyr=26, 
                                  s=20, 
...
p1 <- ggplot(data=run.sim.MSIRS) + geom_line(aes(x=time, y=proportion, color=state)) +
      theme_bw() + theme(panel.grid = element_blank()) + scale_color_manual(values=colz)
#p1

run.sim.MSIRS$model = "MSIRS"

#and run MSIRN
run.sim.MSIRN <- sim.met.MSIRN.age(burnin=30,
                                   sim_pop=10000,
                                   yrs=50,
                                   ntyr=26,
                                   s=20,
...
p2 <- ggplot(data=run.sim.MSIRN) + geom_line(aes(x=time, y=proportion, color=state)) +
  theme_bw() + theme(panel.grid = element_blank()) + scale_color_manual(values=colz2)
#p2

run.sim.MSIRN$model = "MSIRN"

#and run MSIRNI
run.sim.MSIRNI <- sim.met.MSIRN.age(burnin=30,
                                   sim_pop=10000,
                                   yrs=50,
                                   ntyr=26,
                                   s=20,
                                   ...
                                   add.inf.mort = FALSE, 
                                   N_stat="matAB")
run.sim.MSIRNI$model = "MSIRNI"

p3 <- ggplot(data=run.sim.MSIRNI) + geom_line(aes(x=time, y=proportion, color=state)) +
  theme_bw() + theme(panel.grid = element_blank()) + scale_color_manual(values=colz2)
#p3

#join together and plot time series

all.sim <- rbind(run.sim.MSIRN, run.sim.MSIRS,run.sim.MSIRNI)
all.sim$model <- factor(all.sim$model, levels=c("MSIRN","MSIRS", "MSIRNI"))


p4 <- ggplot(data=all.sim) + geom_line(aes(x=time, y=proportion, color=state)) +
  theme_bw() + theme(panel.grid = element_blank(), strip.background = element_rect(fill="white")) + 
  scale_color_manual(values=colz2) + scale_x_continuous(breaks=seq(30,50,5), labels=c(0:4)) +
  facet_grid(model~.) + xlab("years of monitoring")
#p4

ggsave(file = "simModels.png",
       plot=p4,
       units="mm",  
       width=30, 
       height=60, 
