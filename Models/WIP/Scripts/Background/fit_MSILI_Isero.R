...
buildTMat_MSILI_age_titer <- function(c, Npop, age.classes, surv.biwk, age.brk, surv.juv.biwk, mu.sick,	beta, recov, sigma, sero, wane, recru, lat, I_m_titer, add.inf.mort, age.rate, slope.wane){
  ...
  foi = 1-exp(-beta*(sum(I_m)/sum(Npop_epi))) #vector
  #psi = 1-exp(-sero*(sum(I_m)/sum(Npop_epi))) #force of seroconversion, also depends on density of I class
  lsi = 1-exp(-lat*(I_m_titer)) #force of latency. lat is 1/duration of infectious period in biweeks (rate of latency). also depends on antibody titer by age
  sli = 1-exp(-recru*(I_m_titer)) #force of recrudescence. recru is 1/duration of latency in biweeks. also depends  on antibody titer by age
    
  mat1 <- matrix(0,4,4) # MSI
  ...
  for (j in 1:nage) {
    ...
    mat1[4,2] <- 0
    mat1[4,3] <- lsi[j]
    mat1[4,4] <- 1-sli[j] #already included here. simply give sigma a value if you want to allow for waning immunity
    mat1[3,4] <- sli[j]
...
buildTMat_MSILI_age <- function(c, Npop, age.classes, surv.biwk, age.brk, surv.juv.biwk, mu.sick,	beta, recov, sigma, sero, wane, recru, lat,  add.inf.mort, age.rate, slope.wane){
  ...
  if (length(age.rate)==1) age.rate <- rep(age.rate,nage)
  if (length(wane)==1) wane <- rep(wane,nage)
  if (length(recru)==1) recru <- rep(recru,nage)
  if (length(lat)==1) lat <- rep(lat,nage)
  waning.maternal = wane
  ...
  #could later make these probabilities based on titer distribution
  #for now just keep as rates
  # lsi = 1-exp(-lat*(I_m_titer)) #force of latency. lat is 1/duration of infectious period in biweeks (rate of latency). also depends on antibody titer by age
  #sli = 1-exp(-recru*(I_m_titer)) #force of recrudescence. recru is 1/duration of latency in biweeks. also depends  on antibody titer by age
  ...
  for (j in 1:nage) {
    ...
    mat1[3,3] <- 1-lat[j]
    mat1[4,2] <- 0
    mat1[4,3] <- lat[j] #transition from Infectious to Latent
    mat1[4,4] <- 1-recru[j] 
    mat1[3,4] <- recru[j] #recrudescence from Latent to Infectious
...
buildFMatrix_MSILI_Lsero <- function(age.classes, adult_fec, surv.biwk, biwk){ 	#one for every age class
  #base your fertility on the biweek of the year
  #pop will grow a bit because higher chance of surviving to birth when births come earlier
  #but these total to the annual fec rate
  if(biwk==1){ #peak births
  ...
  for (j in 1:nage) {
    Fmat[1,((j-1)*4+1):(j*4)] <- c(0,0,0,fert.biwk[j]) #the mat immune
    Fmat[2,((j-1)*4+1):(j*4)]<- c(fert.biwk[j],fert.biwk[j],fert.biwk[j],0) #the susceptible (mom didn't get sick)
  }
  
  return(Fmat)
}
buildFMatrix_MSILI_Isero <- function(age.classes, adult_fec, surv.biwk, biwk){ 	#one for every age class
  #base your fertility on the biweek of the year
  #pop will grow a bit because higher chance of surviving to birth when births come earlier
  #but these total to the annual fec rate
  if(biwk==1){ #peak births
  ...
  #both I and L class pass ABs to pup
  for (j in 1:nage) {
    Fmat[1,((j-1)*4+1):(j*4)] <- c(0,0,fert.biwk[j],0) #the mat immune
    Fmat[2,((j-1)*4+1):(j*4)]<- c(fert.biwk[j],fert.biwk[j],0,fert.biwk[j]) #the susceptible (mom didn't get sick)
  }
...
sim.met.MSILI.age <- function(burnin, sim_pop, yrs, ntyr, age.brk, s, beta, sero, recov, mort, mort_juv, adult_fec, wane, slope.wane, lat, recru, boost, sigma2, sigma, mu.sick, add.inf.mort, N_stat, model){
  #first pull out your data...
  c=4
  # dat.tmp <- subset(dat.age1, species==species1 & type==type1)
  #then run model for 100 years 
  ...
  N_tot = cbind(M_init, S_init, I_init, L_init)
  ...
  for (i in 1:(length(times)-1)){
    ...
    #and pick titer distribution based on biweek
    #I_m_titer = rnorm(n=s, mean=mean.titer)
    
    Tmat <- buildTMat_MSILI_age(c=c, Npop= N_pop_ts[,i], age.classes=1:s, age.brk=age.brk, surv.biwk = (1-mort)^(1/ntyr), surv.juv.biwk = (1-mort_juv)^(1/ntyr), lat=lat,recru=recru,  mu.sick=mu.sick,	beta=beta, sigma=sigma, recov=recov, sero=sero, wane= wane, add.inf.mort=add.inf.mort, age.rate=1/ntyr, slope.wane=slope.wane)
    #print(i)
    
    #print(biwk1)
    
    #feed into fertility matrix:
    Fmat <- buildFMatrix_MSILI_Lsero(age.classes=1:s, adult_fec =adult_fec, surv.biwk = (1-mort)^(1/ntyr), biwk = biwk1)
  ...
  prop.tot$S = prop.tot$S/prop.tot$N
  prop.tot$I = prop.tot$I/prop.tot$N
  prop.tot$L = prop.tot$L/prop.tot$N
  ...
  dat.tot = data.frame(rbind(cbind(dat.tot[,1],dat.tot[,2]), cbind(dat.tot[,1], dat.tot[,3]), cbind(dat.tot[,1], dat.tot[,4]), cbind(dat.tot[,1], dat.tot[,5])))
  names(dat.tot) = c("time", "count")
  dat.tot$class = rep(c("M", "S", "I", "L"), each= length(times))
  dat.tot$class = factor(dat.tot$class, levels=c("M", "S", "I", "L"))
  
  # colz = c('M'="violet", 'S' = "mediumseagreen", 'I' = 'tomato', 'L' = "cornflowerblue")
  #ggplot(data=dat.tot) + geom_line(aes(x=time, y=count, color=class)) + scale_color_manual(values=colz)
  
  pop.growth = ddply(dat.tot,.(time), summarize, N= sum(count))
  #with(pop.growth, plot(time,N, type="l"))
  dat.tot$N_tot = NA
  for (i in 1:length(pop.growth$time)){
    dat.tot$N_tot[dat.tot$time==pop.growth$time[i]] = pop.growth$N[i]
  }
  dat.tot$proportion = dat.tot$count/dat.tot$N_tot
  #ggplot(data=dat.tot) + geom_line(aes(x=time, y=proportion, color=class)) + scale_color_manual(values=colz) + coord_cartesian(ylim = c(0,1))
  #ggplot(data=dat.tot) + geom_line(aes(x=time, y=proportion, color=class)) + scale_color_manual(values=colz) + coord_cartesian(ylim = c(0,1), xlim=c(49,50))
  ...
  age.dat.tot = cbind.data.frame(rep(times, s*c), age.dat.tot)
  names(age.dat.tot) = c("times", "count")
  age.dat.tot$class = rep(c("M", "S", "I", "L"), each= length(times)*s)
  age.dat.tot$age = rep(rep(seq(0,(s-1),1), each=length(times)), c)
  ...
  eq.dat.sero$age = eq.dat.sero$age + revtrunc(eq.dat.sero$times)
  eq.dat.sero$sero = 0
  #here, I and M...or L and M???
  eq.dat.sero$sero[eq.dat.sero$class=="L" | eq.dat.sero$class=="M"] <- eq.dat.sero$count[eq.dat.sero$class=="L" | eq.dat.sero$class=="M"]
  eq.sero.tot <- ddply(eq.dat.sero, .(times, age), summarize, seropos = sum(sero), N= sum(count))
  eq.sero.tot$seroprevalence = eq.sero.tot$seropos/eq.sero.tot$N
  ...
  #ggplot(data=out.mat) + geom_point(aes(age, seroprevalence, color=biwk), pch=1) +geom_line(aes(age, seroprevalence, color=biwk)) + facet_wrap(~biwk, ncol=4)
  #names(eq.sero.tot)[1] ="biwk"
  #plot one of them to get a sense
  #plot.age.seroprev = list.by.T[[5]]
...
sim.met.MSILI.age.Isero <- function(burnin, sim_pop, yrs, ntyr, age.brk, s, beta, sero, recov, mort, mort_juv, adult_fec, wane, slope.wane, lat, recru, boost, sigma2, sigma, mu.sick, add.inf.mort, N_stat, model){
  #first pull out your data...
  c=4
  # dat.tmp <- subset(dat.age1, species==species1 & type==type1)
  #then run model for 100 years 
  ...
  for (i in 1:(length(times)-1)){
    ...
    #feed into fertility matrix:
    Fmat <- buildFMatrix_MSILI_Isero(age.classes=1:s, adult_fec =adult_fec, surv.biwk = (1-mort)^(1/ntyr), biwk = biwk1)
  ...
  eq.dat.sero$sero = 0
  #here, I and M...or L and M???
  eq.dat.sero$sero[eq.dat.sero$class=="I" | eq.dat.sero$class=="M"] <- eq.dat.sero$count[eq.dat.sero$class=="L" | eq.dat.sero$class=="M"]
  eq.sero.tot <- ddply(eq.dat.sero, .(times, age), summarize, seropos = sum(sero), N= sum(count))
  eq.sero.tot$seroprevalence = eq.sero.tot$seropos/eq.sero.tot$N
...
log.lik.wane.age <- function(par, data, cutoff, burnin, sim_pop, yrs, ntyr, age.brk,  s, beta, sero, boost, recov, mort, mort_juv, adult_fec, wane, slope.wane, sigma, mu.sick, add.inf.mort, model, N_stat){
  ...
  out = sim.met.MSILI.age(burnin=burnin,
                            sim_pop=sim_pop,
                            yrs=yrs,
                            ntyr=ntyr,
                            s=s,
                            ...
                            sigma=sigma,
                            sigma2=sigma2,
                            recru = exp(par[length(par)]),
                            lat = exp(par[(length(par)-1)]),
                            model=model,
                            mu.sick = mu.sick,
                            add.inf.mort = add.inf.mort,
                            N_stat=N_stat)
...
log.lik.wane.age.MSILI <- function(par, data, cutoff, burnin, sim_pop, yrs, ntyr, age.brk,  s, beta, sero, boost, lat, recru, recov, mort, mort_juv, adult_fec, wane, slope.wane, sigma, mu.sick, add.inf.mort, model, N_stat){
...
log.lik.wane.age.MSILI.Isero <- function(par, data, cutoff, burnin, sim_pop, yrs, ntyr, age.brk,  s, beta, sero, boost, lat, recru, recov, mort, mort_juv, adult_fec, wane, slope.wane, sigma, mu.sick, add.inf.mort, model, N_stat){
  ...
  if(model=="MSIR"){
    ...
    out = sim.met.MSILI.age.Isero(burnin=burnin,
                            sim_pop=sim_pop,
                            yrs=yrs,
                            ntyr=ntyr,
                            s=s,
...
wrap.msili.fit.Isero <- function(model, dat1, cutoff1, species1, type1, vis_split, s, sim_pop, yrs, ntyr, recov, lat, recru, sero, wane, sigma, sigma2, boost, mort, mort_juv, N_stat,
                                              adult_fec, mu.sick, add.inf.mort, beta, age.brk, slope.wane, burnin, do.plot, do.save, obs.proc, filename){
  
  #first pull out your data...
  dat.tmp <- subset(dat1, species==species1 & type==type1)
  dat.tmp <- subset(dat.tmp, !is.na(age))         # 109
  #dat.tmp <- subset(dat.tmp, fit==1) #remove the # 72
  ...
  if(model=="MSIR" | model=="MSI"){
    ...
    par = c(wane, beta, lat, recru) 
  }
  print(par)
  #estimate parameters
  
  out.lik <- optim(par=log(par),fn=log.lik.wane.age.MSILI.Isero, method="Nelder-Mead", data=dat.tmp, model=model,  burnin=burnin, sigma=sigma, boost=boost, recru=recru, lat = lat,
                   cutoff=cutoff1, s=s, sim_pop=sim_pop, ntyr=ntyr, yrs=yrs, wane=wane, recov=recov, slope.wane=slope.wane, sero=sero, age.brk=age.brk,
                   mort=mort, mort_juv=mort_juv, adult_fec=adult_fec, mu.sick=mu.sick, add.inf.mort=add.inf.mort, N_stat=N_stat,
                   control=list(maxit=1000), hessian = TRUE)
  ...
  if(model=="MSIR"){
    ...
    wane = exp(out.lik$par[1])
    beta = exp(out.lik$par[2:(length(par)-2)])
    lat = exp(out.lik$par[(length(par)-1)])
    recru = exp(out.lik$par[length(par)])
    
    ConfidenceInterval$par[1] <- "wane"
    ConfidenceInterval$par[2:(length(par)-2)] <- "beta"
    ConfidenceInterval$par[(length(par)-1)] <- "lat"
    ConfidenceInterval$par[length(par)] <- "recru"
  ...
  beta_lci <- beta_uci <- wane_lci <- wane_uci <- sigma_lci <- sigma_uci <- sigma2_lci <- sigma2_uci <- sero_lci <- sero_uci <- boost_lci <- boost_uci <- recru_lci <- recru_uci <- lat_lci <- lat_uci <- 0
  ...
  if (model=="MSIR"){
    ...
    for (j in 1:length(par.dist[,1])) {
      ...
      lat_lci <- ConfidenceInterval$lower[ConfidenceInterval$par=="lat"]
      lat_uci <- ConfidenceInterval$upper[ConfidenceInterval$par=="lat"]
      
      recru_lci <- ConfidenceInterval$lower[ConfidenceInterval$par=="recru"]
      recru_uci <- ConfidenceInterval$upper[ConfidenceInterval$par=="recru"]
      
      
      model.out  = sim.met.MSILI.age.Isero(burnin=burnin,
                                     sim_pop=sim_pop,
                                     yrs=yrs,
                                     ntyr=ntyr,
                                     s=s,
      ...
      for (j in 1:length(par.dist[,1])) {
        ...
        tmp  = sim.met.MSILI.age.Isero(burnin=burnin,
                                 sim_pop=sim_pop,
                                 yrs=yrs,
                                 ntyr=ntyr,
                                 s=s,
  ...
  fit.stat <- data.frame(cbind(species1, type1, mort, mort_juv, adult_fec,  recov, beta, beta_lci, beta_uci, sigma, sigma_lci, sigma_uci, sigma2, sigma2_lci, sigma2_uci, wane, wane_lci, wane_uci, boost, boost_lci, boost_uci, sero, sero_lci, sero_uci, lat, lat_lci, lat_uci, recru, recru_lci, recru_uci, mu.sick, llik, convergence))
  names(fit.stat) <- c("species", "type", "mort", "juv_mort", "adult_fec", "recov", "beta", "beta_lci", "beta_uci", "sigma", "sigma_lci", "sigma_uci", "sigma2", "sigma2_lci", "sigma2_uci", "wane", "wane_lci", "wane_uci", "boost", "boost_lci", "boost_uci", "sero", "sero_lci", "sero_uci", "lat", "lat_lci", "lat_uci", "recru", "recru_lci", "recru_uci", "mu_sick","neg_llik", "convergence")
  fit.stat$AIC = AIC
  fit.stat$AICc = AICc
  fit.stat$k = k 
  fit.stat$n = n
...
# mort = .489; mort_juv= .456; adult_fec = .48
                      ...
                      fit.Eid.mean.MSILI.Ifit <- wrap.msili.fit.Isero(model="MSILI",
                                                           dat1=dat.tot,
...
save(fit.Eid.mean.MSILI.Ifit, file="fit.Eid.mean.MSILI.Ifit.Rdata")

 #and run for lci and uci
 fit.Eid.lci.MSILI.Ifit <- wrap.msili.fit.Isero(model="MSILI",
                                      dat1=dat.tot,
                                      species1 = "Eidolon dupreanum",
                                      type1="NIVG",
                                      cutoff1 = "lci",
...
# 
# 
 save(fit.Eid.lci.MSILI.Ifit, file="fit.Eid.lci.MSILI.Ifit.Rdata")
# 
# 
# 
# #and run for lci and uci
  fit.Eid.uci.MSILI.Ifit <- wrap.msili.fit.Isero(model="MSILI",
                                     dat1=dat.tot,
                                     species1 = "Eidolon dupreanum",
                                     type1="NIVG",
                                     cutoff1 = "uci",
...
# 
# 
 save(fit.Eid.uci.MSILI.Ifit, file="fit.Eid.uci.MSILI.Ifit.Rdata")
