...
# Accomplishments
# 02-07-24 Set up flexible metapopulation structure for MSIRS - input num subs
# 02-12-24 Cleaned up - altered code to take model input number and loop through simulations and models
...
# fecundity matrix
buildFMatrix_MSIRN <- function(age.classes, fecundity, surv.biwk, biwk){ 	
...
sim.met <- function(model, num_subpop, burnin, sim_pop, yrs, ntyr, age.brk, s, param.array, add.inf.mort, printpop, stoch_disease, stoch_demo){
  #################################
  ...
  mat1 = build.pop.mat(surv=(1-mort_ad[i]), surv_juv=(1-mort_juv[i]), s=(s), fecundity = fecundity[i])
  ...
  for (j in 1:(length(times)-1)){
    ...
    biwk1 <- find.biweek(t=j, times=times)
    
    if(model==1){
      # generate SIR matrix
      Tmat <- buildTMat_age(model = model, stoch_disease = stoch_disease, param.array = param.array, c = c, Npop = N_pop_ts[[i]][,j], age.classes = 1:s, age.brk = age.brk, surv.biwk = (1-mort_ad[i])^(1/ntyr), surv.juv.biwk = (1-mort_juv[i])^(1/ntyr), add.inf.mort = add.inf.mort, age.rate = 1/ntyr)
      
      # generate fecundity matrix
      Fmat <- buildFMatrix(age.classes = 1:s, fecundity = fecundity[i], surv.biwk = (1-mort_ad[i])^(1/ntyr), biwk = biwk1)
      ...
      # generate SIR matrix
      Tmat <- buildTMat_MSIRN_age(model = model, stoch_disease = stoch_disease, param.array = param.array, c = c, Npop = N_pop_ts[[i]][,j], age.classes = 1:s, age.brk = age.brk, surv.biwk = (1-mort_ad[i])^(1/ntyr), surv.juv.biwk = (1-mort_juv[i])^(1/ntyr), add.inf.mort = add.inf.mort, age.rate = 1/ntyr)
      
      # generate fecundity matrix
      Fmat <- buildFMatrix_MSIRN(age.classes = 1:s, fecundity = fecundity[i], surv.biwk = (1-mort_ad[i])^(1/ntyr), biwk = biwk1)
  ...
  for(i in 1:num_subpop){
    ...
    #############################
    stab.struct <- get.age.struct.M(pop=N_pop_ts[[i]][,ncol(N_pop_ts[[i]])], c=c)
    stab.struct <- c(unlist(stab.struct))
    stab.struct <- stab.struct/(sum(stab.struct))
    #plot(stab.struct, xlab="Age", ylab="Proportion")  
    
    tmp <- transform.vect(vec=N_pop_ts[[i]], s=s, c=c)

    ######################
    ## Sum age classes
    #####################
    # split matrix into age classes
    N_split_pop = mat_split(tmp, r=c, c=ncol(tmp))
    ...
    N_split_disease = mat_split(tmp, r=s, c=ncol(tmp))
...
simulations <- function(model, sim_pop, numsims, num_subpop, param.array, burnin, yrs, do.plot){
  ...
  if(do.plot==TRUE){
    ...
    if(model == 1){
      ...
      p1 <<- ggplot(data=sim.summary) + geom_line(aes(x=time, y=mean.proportion, color=state)) + facet_grid(~subpop) +
        ...
        scale_x_continuous(breaks=seq(0, max(sim.summary$time), 1)) + ggtitle(paste0("MSIR. initpop = ", sim_pop[1], " numsims = ", numsims)) +
        xlab("Year")
      print(p1)
      
      ggsave(file = paste0("Model", model, "initpop = ", sim_pop[model], " numsims = ", numsims, " yrs = ", yrs, Sys.time(), ".png"),
             plot=p1,
             units="mm",  
             width=70, 
             height=60, 
    ...
    if(model == 2){
      ...
      p2 <<- ggplot(data=sim.summary) + geom_line(aes(x=time, y=mean.proportion, color=state)) + facet_grid(~subpop) +
        theme_bw() + theme(panel.grid = element_blank(), strip.background = element_rect(fill="white")) + 
        geom_ribbon(aes(x=time, ymin = lower.ci.proportion, ymax = upper.ci.proportion, fill = state), alpha = 0.2) +
        scale_color_manual(values=colz) +
        scale_x_continuous(breaks=seq(0, max(sim.summary$time), 1)) + ggtitle(paste0("MSIRN. initpop = ", sim_pop[2], " numsims = ", numsims)) +
        xlab("Year")
      print(p2)
    ...
    if( model == 3){
      colz = c('M'="violet", 'S' = "mediumseagreen", 'I' = 'tomato', 'R' = "cornflowerblue", 'N' = "navy")
      p3 <<- ggplot(data=sim.summary) + geom_line(aes(x=time, y=mean.proportion, color=state)) + facet_grid(~subpop) +
        theme_bw() + theme(panel.grid = element_blank(), strip.background = element_rect(fill="white")) + 
        geom_ribbon(aes(x=time, ymin = lower.ci.proportion, ymax = upper.ci.proportion, fill = state), alpha = 0.2) +
        scale_color_manual(values=colz) +
        scale_x_continuous(breaks=seq(0, max(sim.summary$time), 1)) + ggtitle(paste0("MSIRNI. initpop = ", sim_pop[3], " numsims = ", numsims)) +
        xlab("Year")
     print(p3)
...
#############################
sim.all.models <- function(yrs, numsims){
  for (i in 1:3){
...
sim.all.models(yrs = 10, numsims = 10)  
