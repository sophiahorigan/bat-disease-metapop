...
##########
# transmission matrix 
buildTMat_age <- function(c, Npop, age.classes, surv.biwk, age.brk, surv.juv.biwk, mu.sick,	beta, recov, sigma, wane, add.inf.mort, age.rate){
...
# transmission matrix
buildTMat_MSIRN_age <- function(c, Npop, age.classes, surv.biwk, age.brk, surv.juv.biwk, mu.sick, rho, beta, recov, sigma, wane, add.inf.mort, age.rate){
...
# simulate model
sim.met.MSIRN.age <- function(burnin, sim_pop, yrs, ntyr, s, beta, age.brk, recov, mort, mort_juv, adult_fec, wane, rho, sigma, mu.sick, add.inf.mort, N_stat, printpop){
  ...
  for (i in 1:(length(times)-1)){
    ...
    # generate SIR matrix
    Tmat <- buildTMat_MSIRN_age(c=c, Npop= N_pop_ts[,i], age.classes=1:s, age.brk = age.brk, surv.biwk = (1-mort)^(1/ntyr), surv.juv.biwk = (1-mort_juv)^(1/ntyr), mu.sick=mu.sick, rho=rho,	beta=beta, sigma=sigma, recov=recov, wane= wane, add.inf.mort=add.inf.mort, age.rate=1/ntyr)
...
run.sim.MSIRNI <- sim.met.MSIRN.age(burnin=0,
                                    sim_pop=10000,
                                    yrs=10,
                                    ntyr=26,
                                    s=20,
