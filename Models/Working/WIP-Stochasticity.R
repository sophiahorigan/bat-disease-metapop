...
# To Do
# make it easy to run many simulations at once
...
test <- simulations(model = 1, numsims = 100, param.array = param.array, burnin = 0, sim_pop = 10000, yrs = 1)



colz = c('M'="violet", 'S' = "mediumseagreen", 'I' = 'tomato', 'R' = "cornflowerblue")

p1_msirs <- ggplot(data=test) + geom_point(aes(x=time, y=proportion, color=state)) + 
  theme_bw() + theme(panel.grid = element_blank(), strip.background = element_rect(fill="white")) + 
  scale_color_manual(values=colz) +
  scale_x_continuous(breaks=seq(0, max(run.sim.MSIRS$time), 1)) + ggtitle("MSIR") +
  xlab("Year")
